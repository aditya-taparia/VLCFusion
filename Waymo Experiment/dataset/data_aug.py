...
def dbsample(CLASSES, CLASSES_TO_NAMES, data_root, data_dict, db_sampler, sample_groups):
    '''
    CLASSES: dict(Pedestrian=0, Cyclist=1, Car=2, Sign=3)
    CLASSES_TO_NAMES: dict(PEDESTRIAN='Pedestrian', CYCLIST='Cyclist', VEHICLE='Car', SIGN='Sign')
    ...
    for name, v in sample_groups.items():
        ...
        for i in range(n_gt, len(coll_mat)):
            if any(coll_mat[i]):
                ...
                # remove SIGN and difficulty=-1
                if cur_sample['name'] == 'SIGN' or cur_sample['difficulty'] == -1:
                    continue

                sampled_pts.append(sampled_pts_cur)
                sampled_names.append(CLASSES_TO_NAMES[cur_sample['name']])
                sampled_labels.append(CLASSES[CLASSES_TO_NAMES[cur_sample['name']]])
    ...
    if len(sampled_bboxes) > 0:
        ...
        gt_difficulty = np.concatenate([gt_difficulty, np.array(sampled_difficulty)], axis=0)
    data_dict = {
            'pts': pts,
...
def data_augment(CLASSES, CLASSES_TO_NAMES, data_root, data_dict, data_aug_config):
    '''
    CLASSES: dict(Pedestrian=0, Cyclist=1, Car=2, Sign=3)
    CLASSES_TO_NAMES: dict(PEDESTRIAN='Pedestrian', CYCLIST='Cyclist', VEHICLE='Car', SIGN='Sign')
    data_root: str, data root
