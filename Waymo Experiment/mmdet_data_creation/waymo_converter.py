...
class Waymo2KITTI(object):
    ...
    def convert_one(self, frame_idx):
        ...
        Args:
            frame_idx (int): Index of the frame to be converted.
        ...
        for f_idx, data in enumerate(frame_data):
            frame = data
            
            # Step 1. Extract camera images and lidar point clouds from waymo 
            # raw data and save as kitti format.
            if self.save_senor_data:
                self.save_image(frame, frame_idx, f_idx)
                self.save_lidar(frame, frame_idx, f_idx)
            
            # Step 2. Generate waymo train/val/test infos and save as pkl file.
            # TODO save the depth image for waymo challenge solution.
            self.create_waymo_info_file(frame, frame_idx, f_idx, file_infos)
        return file_infos
    ...
    def save_image(self, frame, file_idx, frame_idx):
        ...
        for img in frame.images:
            img_path = f'{self.image_save_dir}{str(img.name - 1)}/' + \
                f'{self.prefix}{str(frame_idx).zfill(3)}' + \
                f'{str(file_idx).zfill(3)}.jpg'
            # with open(img_path, 'wb') as fp:
            #     fp.write(img.image)
            img = cv2.imdecode(np.frombuffer(img.image, np.uint8), cv2.IMREAD_COLOR)
            rgb_img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
            plt.imsave(img_path, rgb_img, format='jpg')
    ...
    def save_lidar(self, frame, file_idx, frame_idx):
        ...
        pc_path = f'{self.point_cloud_save_dir}/{self.prefix}' + \
            f'{str(frame_idx).zfill(3)}{str(file_idx).zfill(3)}.bin'
    ...
    def create_waymo_info_file(self, frame, file_idx, frame_idx, file_infos):
        ...
        # Gather frame infos
        sample_idx = \
            f'{self.prefix}{str(frame_idx).zfill(3)}{str(file_idx).zfill(3)}'
