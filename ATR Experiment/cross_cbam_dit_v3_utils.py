...
class CrossCBAMDiTFusionV3(nn.Module):
    def __init__(
        self,
        mod1_channels: int,
        mod2_channels: int,
    ...
    def forward(
        ...
        s1 = self.film_mod_self1(s1,cond)
        # s1 = self.self_activation1(s1)
        s1 = self.self_cbam1(s1) + x1
        
        # Modality 2 Self Attention
        s2 = self.norm_self2(x2)
        s2 = self.film_mod_self2(s2,cond)
        # s2 = self.self_activation2(s2)
        s2 = self.self_cbam2(s2) + x2
        
        # Fusion
        concat = torch.cat([s1, s2], dim=1)
        fused_base = self.fusion_bottleneck(concat)
        f = self.norm_fusion_bottleneck(fused_base)
        f = self.film_mod_fusion_bottleneck(f, cond)
        # f = self.fusion_bottleneck_activation(f)
        f = self.self_cbam_fusion(f) + fused_base
        
        # Fusion Feedforward Network
        f_ffn = self.norm_fusion_ffn(f)
        f_ffn = self.film_mod_fusion_ffn(f_ffn, cond)
        f_ffn = self.fusion_ffn(f_ffn) + f
        
        # f_refined = self.fusion_out(f)
        # f_refined = self.film_mod_fusion_out(f_refined, cond)
        # # f_refined = self.fusion_out_activation(f_refined)
        # f_refined = self.fusion_out_cbam(f_refined) + f
        # return f_refined
...
class CrossCBAMDiTTransformLayerV3(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        super().__init__()
        self.fusion = CrossCBAMDiTFusionV3(
...
class CrossCBAMDiTTransformQueriesV3(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
if __name__ == "__main__":
    ...
    # --- Shape tests ---
    fusion = CrossCBAMDiTFusionV3(
        mod1_channels=C, mod2_channels=C, out_channels=C, cond_dim=cond_dim, r=2,
    )
    out = fusion(x1, x2, cond)
    print(f"CrossCBAMDiTFusionV3: ({x1.shape}, {x2.shape}) + cond -> {out.shape}")
    assert out.shape == (B, C, H, W)

    in_ch, out_ch = 512, 256
    x_cat = torch.randn(B, in_ch, H, W)
    layer = CrossCBAMDiTTransformLayerV3(in_ch, out_ch, cond_dim=cond_dim, r=2)
    out_layer = layer(x_cat, cond)
    print(f"CrossCBAMDiTTransformLayerV3: {x_cat.shape} -> {out_layer.shape}")
    assert out_layer.shape == (B, out_ch, H, W)

    queries = CrossCBAMDiTTransformQueriesV3(in_ch, out_ch, cond_dim=cond_dim, r=2)
    x_q = torch.randn(B, in_ch, 1, 1)
    out_q = queries(x_q, cond)
    print(f"CrossCBAMDiTTransformQueriesV3: {x_q.shape} -> {out_q.shape}")
    ...
    print("All CrossCBAM-DiT V3 modules passed.")
