...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, kernel_size: int = 7, dilation: int = 1):
        ...
        self.conv = nn.Conv2d(2, 1, kernel_size=kernel_size,
                              padding=padding, stride=1, dilation=dilation, bias=False)
...
class CBAM(nn.Module):
    """CBAM: channel attention then spatial attention (no norm, no residual)."""
    def __init__(self, channels: int, r: int = 2, kernel_size: int = 7, dilation: int = 1):
        super().__init__()
        self.cam = ChannelAttention(channels, r)
        self.sam = SpatialAttention(kernel_size=kernel_size, dilation=dilation)
...
class MultiHeadCBAM(nn.Module):
    """
    Full-input multi-head CBAM with learned average over refined features.
    """
    def __init__(
        self,
        channels: int,
        head_specs=((2, 3, 1), (4, 5, 1), (8, 7, 1), (16, 9, 1)),
    ):
        super().__init__()
        self.heads = nn.ModuleList([
            CBAM(channels, r=r, kernel_size=k, dilation=d)
            for (r, k, d) in head_specs
        ])
        self.num_heads = len(self.heads)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ys = [head(x) for head in self.heads]          # list of [B,C,H,W]
        y = torch.stack(ys, dim=1)                    # [B,K,C,H,W]
        
        y = y.mean(dim=1)                             # [B,C,H,W]
        return y
...
class VLCBlock(nn.Module):
    ...
    def __init__(
        ...
        self.cbam = MultiHeadCBAM(
            channels,
            head_specs=((2,3,1), (2,5,1), (2,7,1)),
        )
    ...
    # V1 had a learned average over the heads
    # V2 has a mean over the heads
    # V3 has a mean over the heads and a ((2,3,1), (2,5,1), (2,7,1)) head_specs.
...
class CrossCBAMDiTFusionV7(nn.Module):
    """
    x1 -> VLCBlock -> s1
    x2 -> VLCBlock -> s2
...
class CrossCBAMDiTTransformLayerV7(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        super().__init__()
        self.fusion = CrossCBAMDiTFusionV7(
...
class CrossCBAMDiTTransformQueriesV7(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
if __name__ == "__main__":
    ...
    # --- Shape tests ---
    fusion = CrossCBAMDiTFusionV7(
        mod1_channels=C, mod2_channels=C, out_channels=C, cond_dim=cond_dim, r=2,
    )
    out = fusion(x1, x2, cond)
    print(f"CrossCBAMDiTFusionV7: ({x1.shape}, {x2.shape}) + cond -> {out.shape}")
    assert out.shape == (B, C, H, W)

    in_ch, out_ch = 512, 256
    x_cat = torch.randn(B, in_ch, H, W)
    layer = CrossCBAMDiTTransformLayerV7(in_ch, out_ch, cond_dim=cond_dim, r=2)
    out_layer = layer(x_cat, cond)
    print(f"CrossCBAMDiTTransformLayerV7: {x_cat.shape} -> {out_layer.shape}")
    assert out_layer.shape == (B, out_ch, H, W)

    queries = CrossCBAMDiTTransformQueriesV7(in_ch, out_ch, cond_dim=cond_dim, r=2)
    x_q = torch.randn(B, in_ch, 1, 1)
    out_q = queries(x_q, cond)
    print(f"CrossCBAMDiTTransformQueriesV7: {x_q.shape} -> {out_q.shape}")
    ...
    print("All CrossCBAM-DiT V7 modules passed.")
