...
# =============================================================================
# VLC Block — the reusable primitive
# =============================================================================
...
class VLCBlock(nn.Module):
    ...
    def __init__(
        ...
        if if_ffn:
            ...
            self.film_ffn = FiLMModulation(channels, cond_dim)
    ...
    def forward(self, x: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        ...
        if self.if_ffn:
            ...
            g2, b2, a2 = self.film_ffn(h2, cond)
            h3 = (1 + g2) * h2 + b2
            out2 = a2 * self.ffn(h3) + out1
            return out2
        else:
            return out1
...
class CrossCBAMDiTFusionV5(nn.Module):
    """
    x1 -> VLCBlock -> s1
    x2 -> VLCBlock -> s2
    concat(s1, s2) -> fusion_bottleneck -> VLCBlock (with FFN) -> output
    """
    ...
    def __init__(
        ...
        self.vlc_mod1 = VLCBlock(mod1_channels, cond_dim, r=r, num_groups=num_groups, if_ffn=False, ffn_factor=2, ffn_kernel_size=3)
        self.vlc_mod2 = VLCBlock(mod2_channels, cond_dim, r=r, num_groups=num_groups, if_ffn=False, ffn_factor=2, ffn_kernel_size=3)
        ...
        self.vlc_fused = VLCBlock(out_channels, cond_dim, r=r, num_groups=num_groups, if_ffn=True, ffn_factor=4, ffn_kernel_size=3) 
        # In 5.2 we use ffn_factor=2, ffn_kernel_size=3
        # In 5.3 we use ffn_factor=4, ffn_kernel_size=3
        # In 5.4 we use ffn_factor=2, ffn_kernel_size=3, and if_ffn=False for mod1 and mod2
        # [Worked] In 5.5 we use ffn_factor=4, ffn_kernel_size=3, and if_ffn=True for mod1 and mod2 with ffn_factor=2 and ffn_kernel_size=3 with "--warmup_ratio", "0.1", and "--max_grad_norm", "1.0"
        # In 5.6 we use ffn_factor=4, ffn_kernel_size=3, and if_ffn=True for mod1 and mod2 with ffn_factor=1 and ffn_kernel_size=3 with "--warmup_ratio", "0.1", and "--max_grad_norm", "1.0"
        # In 5.7 we use ffn_factor=4, ffn_kernel_size=3, and if_ffn=True for mod1 and mod2 with ffn_factor=1 and ffn_kernel_size=3 with "--warmup_ratio", "0.1", and "--max_grad_norm", "1.0" and condition indices 16,13,1,11,15,19
        # In metaclip version we use ffn_factor=4, ffn_kernel_size=3, and if_ffn=True for mod1 and mod2 with ffn_factor=2 and ffn_kernel_size=3 with "--warmup_ratio", "0.1", and "--max_grad_norm", "1.0"
        # In metaclip version v2 we use ffn_factor=4, ffn_kernel_size=3, and if_ffn=False for mod1 and mod2 with "--warmup_ratio", "0.1", and "--max_grad_norm", "1.0"
    ...
    def forward(
        ...
        return self.vlc_fused(fused, cond)
...
class CrossCBAMDiTTransformLayerV5(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        super().__init__()
        self.fusion = CrossCBAMDiTFusionV5(
...
class CrossCBAMDiTTransformQueriesV5(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
if __name__ == "__main__":
    ...
    # --- Shape tests ---
    fusion = CrossCBAMDiTFusionV5(
        mod1_channels=C, mod2_channels=C, out_channels=C, cond_dim=cond_dim, r=2,
    )
    out = fusion(x1, x2, cond)
    print(f"CrossCBAMDiTFusionV5: ({x1.shape}, {x2.shape}) + cond -> {out.shape}")
    assert out.shape == (B, C, H, W)

    in_ch, out_ch = 512, 256
    x_cat = torch.randn(B, in_ch, H, W)
    layer = CrossCBAMDiTTransformLayerV5(in_ch, out_ch, cond_dim=cond_dim, r=2)
    out_layer = layer(x_cat, cond)
    print(f"CrossCBAMDiTTransformLayerV5: {x_cat.shape} -> {out_layer.shape}")
    assert out_layer.shape == (B, out_ch, H, W)

    queries = CrossCBAMDiTTransformQueriesV5(in_ch, out_ch, cond_dim=cond_dim, r=2)
    x_q = torch.randn(B, in_ch, 1, 1)
    out_q = queries(x_q, cond)
    print(f"CrossCBAMDiTTransformQueriesV5: {x_q.shape} -> {out_q.shape}")
    ...
    print("All CrossCBAM-DiT V5 modules passed.")
