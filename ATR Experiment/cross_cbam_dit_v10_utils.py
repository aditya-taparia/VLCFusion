...
class CrossCBAMDiTFusionV10(nn.Module):
    """
    x1 -> VLCBlock -> s1
    x2 -> VLCBlock -> s2
...
class CrossCBAMDiTTransformLayerV10(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        super().__init__()
        self.fusion = CrossCBAMDiTFusionV10(
...
class CrossCBAMDiTTransformQueriesV10(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
if __name__ == "__main__":
    ...
    # --- Shape tests ---
    fusion = CrossCBAMDiTFusionV10(
        mod1_channels=C, mod2_channels=C, out_channels=C, cond_dim=cond_dim, r=2,
    )
    out = fusion(x1, x2, cond)
    print(f"CrossCBAMDiTFusionV10: ({x1.shape}, {x2.shape}) + cond -> {out.shape}")
    assert out.shape == (B, C, H, W)

    in_ch, out_ch = 512, 256
    x_cat = torch.randn(B, in_ch, H, W)
    layer = CrossCBAMDiTTransformLayerV10(in_ch, out_ch, cond_dim=cond_dim, r=2)
    out_layer = layer(x_cat, cond)
    print(f"CrossCBAMDiTTransformLayerV10: {x_cat.shape} -> {out_layer.shape}")
    assert out_layer.shape == (B, out_ch, H, W)

    queries = CrossCBAMDiTTransformQueriesV10(in_ch, out_ch, cond_dim=cond_dim, r=2)
    x_q = torch.randn(B, in_ch, 1, 1)
    out_q = queries(x_q, cond)
    print(f"CrossCBAMDiTTransformQueriesV10: {x_q.shape} -> {out_q.shape}")
    ...
    print("All CrossCBAM-DiT V10 modules passed.")
