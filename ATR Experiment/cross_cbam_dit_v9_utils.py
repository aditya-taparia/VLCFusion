...
class LocalChannelAttention(nn.Module):
    """Avg + max pool -> shared Conv2d -> sigmoid gate."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Conv1d(1, 1, k) expects (N, 1, L) -> treat channel as sequence
        y = self.conv(avg.unsqueeze(1)) + self.conv(mx.unsqueeze(1))  # [b, 1, c]
        w = torch.sigmoid(y.squeeze(1)).view(b, c, 1, 1)              # [b, c, 1, 1]
...
class LCBAM(nn.Module):
    """LCBAM: Local-channel attention followed by spatial attention."""
    def __init__(self, kernel_size: int = 5):
        super().__init__()
        self.lcam = LocalChannelAttention(kernel_size)
        self.sam = SpatialAttention()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.sam(self.lcam(x))
...
class VLCBlock(nn.Module):
    ...
    def __init__(
        ...
        self.lcamb = LCBAM(kernel_size=5)
...
class CrossCBAMDiTFusionV9(nn.Module):
    """
    x1 -> VLCBlock -> s1
    x2 -> VLCBlock -> s2
    ...
    # V1 is with just fusion after concat with new CBAM block.

# =============================================================================
# Transform wrappers (drop-in for MultimodalDetr: concat input -> single output)
# =============================================================================

class CrossCBAMDiTTransformLayerV9(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        super().__init__()
        self.fusion = CrossCBAMDiTFusionV9(
...
class CrossCBAMDiTTransformQueriesV9(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
if __name__ == "__main__":
    ...
    # --- Shape tests ---
    fusion = CrossCBAMDiTFusionV9(
        mod1_channels=C, mod2_channels=C, out_channels=C, cond_dim=cond_dim, r=2,
    )
    out = fusion(x1, x2, cond)
    print(f"CrossCBAMDiTFusionV9: ({x1.shape}, {x2.shape}) + cond -> {out.shape}")
    assert out.shape == (B, C, H, W)

    in_ch, out_ch = 512, 256
    x_cat = torch.randn(B, in_ch, H, W)
    layer = CrossCBAMDiTTransformLayerV9(in_ch, out_ch, cond_dim=cond_dim, r=2)
    out_layer = layer(x_cat, cond)
    print(f"CrossCBAMDiTTransformLayerV9: {x_cat.shape} -> {out_layer.shape}")
    assert out_layer.shape == (B, out_ch, H, W)

    queries = CrossCBAMDiTTransformQueriesV9(in_ch, out_ch, cond_dim=cond_dim, r=2)
    x_q = torch.randn(B, in_ch, 1, 1)
    out_q = queries(x_q, cond)
    print(f"CrossCBAMDiTTransformQueriesV9: {x_q.shape} -> {out_q.shape}")
    ...
    print("All CrossCBAM-DiT V9 modules passed.")
